...
`settings.tsx` gets the equivalent check at its top-level component — but scoped to `dataMode === "production"` only, not unconditional. `settingsSectionsForMode("demo")` (`-settings-sections.ts:18-26`) already documents (and is tested: `-settings-sections.test.ts`, "shows every section in demo") that demo mode deliberately shows every section to every persona, so any role can be tried out. An unconditional gate would silently take that away from the Manager and Staff demo personas, contradicting an already-documented behavior — the exact mistake `admin.tsx` already avoids by splitting `DemoAdminConsole` (ungated) from `ProductionAdminConsole` (gated). `settings.tsx` isn't split into two components the same way, so the fix is a `dataMode === "production" && !isCurrentUserAdmin` condition rather than a bare `!isCurrentUserAdmin` one.
