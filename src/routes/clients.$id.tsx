...
function ClientProfilePage() {
  ...
  return (
    <>
      <TopBar title={company.name} subtitle={`BR ${company.brNumber} · CR ${company.crNumber} · Incorporated ${formatDate(company.incorporated)}`} />
      ...
      <main className="grid flex-1 grid-cols-1 gap-6 p-6 lg:grid-cols-3">
        <div className="space-y-6 lg:col-span-2">
          ...
          <div className="rounded-xl border border-border bg-card p-5">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-display text-xl font-semibold text-foreground">{company.name}</p>
                  <p className="text-xs text-muted-foreground">{company.package} package · {company.team}</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <StatusPill tone={company.paymentStatus === "Paid" ? "green" : company.paymentStatus === "Pending" ? "yellow" : "red"}>Payment · {company.paymentStatus}</StatusPill>
            ...
            <div className="mt-5 grid grid-cols-2 gap-4 border-t border-border pt-4 text-sm md:grid-cols-4">
              <div>
                <p className="text-[10px] uppercase tracking-wider text-muted-foreground">AR deadline</p>
              ...
              <div>
                ...
                <p className="mt-1 font-medium text-foreground">HKD {company.invoiceAmount.toLocaleString()}</p>
              </div>
              <div>
                <p className="text-[10px] uppercase tracking-wider text-muted-foreground">Active case</p>
                {activeCase ? (
                  <Link to="/annual-returns/$id" params={{ id: activeCase.id }} className="mt-1 inline-block text-sm font-medium text-primary hover:underline">
                    Open case →
                  </Link>
          ...
          <div className="rounded-xl border border-border bg-card">
            <div className="border-b border-border px-5 py-3">
              <h2 className="font-display text-base font-semibold text-foreground">Annual return history</h2>
            </div>
            <ul className="divide-y divide-border">
              {history.map((c) => (
                <li key={c.id} className="flex items-center justify-between px-5 py-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Link to="/annual-returns/$id" params={{ id: c.id }} className="text-xs text-primary hover:underline">Open</Link>
                  </div>
                </li>
              ))}
            </ul>
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            <div className="rounded-xl border border-border bg-card p-5">
              ...
              {activeCase?.checklist.slice(0, 4).map((i) => (
                <div key={i.id} className="flex items-center justify-between border-t border-border py-2 text-sm first:border-t-0">
                  <span className="text-foreground">{i.label}</span>
                  <StatusPill tone={i.received ? "green" : "yellow"}>{i.received ? "Received" : "Missing"}</StatusPill>
                </div>
              ))}
            </div>
            ...
            <div className="rounded-xl border border-border bg-card p-5">
              ...
              <div className="text-sm">
                ...
                <div className="flex justify-between border-b border-border py-2">
                  ...
                  <StatusPill tone={company.paymentStatus === "Paid" ? "green" : company.paymentStatus === "Pending" ? "yellow" : "red"}>{company.paymentStatus}</StatusPill>
                </div>
