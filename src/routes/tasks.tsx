...
export const Route = createFileRoute("/tasks")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Personal and team task list — chase documents, prepare filings, follow up payments." },
    ],
  }),
  component: TasksPage,
});
...
function TasksPage() {
  ...
  return (
    <>
      <TopBar title="Tasks" subtitle={`${mine.filter((t) => !t.done).length} open · ${others.length} across team`} />
      <main className="grid flex-1 grid-cols-1 gap-6 p-6 lg:grid-cols-2">
        {[{ label: "My tasks", list: mine }, { label: "Team tasks", list: others }].map((group) => (
              ...
              {group.list.map((t) => {
                ...
                return (
                  <li key={t.id} className="flex items-start gap-3 px-5 py-3">
                    <input type="checkbox" defaultChecked={t.done} className="mt-1 h-4 w-4 rounded border-border" />
                    <div className="min-w-0 flex-1">
                      <p className={cn("text-sm font-medium", t.done ? "text-muted-foreground line-through" : "text-foreground")}>
                        {t.title}
                      </p>
                      <p className="text-xs text-muted-foreground">{t.companyName} · {assignee.name}</p>
                    </div>
                    <div className="flex items-center gap-2">
                      <StatusPill tone={t.priority === "High" ? "red" : t.priority === "Normal" ? "yellow" : "neutral"}>
                        {t.priority}
                      </StatusPill>
